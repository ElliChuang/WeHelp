...
def signup():
    ...
    account_name = request.form["account_name"]
    secret_code = request.form["secret_code"]
    check_user = mycursor.execute("SELECT username FROM member where username = %s", (value))
    if check_user != None:
        return redirect(url_for("error", message = "帳號已經被註冊"))
    else:
        new_user = "INSERT INTO member (name, username, password) VALUES (%s, %s, %s)"
        new_value = (name, account_name, secret_code)
        mycursor.execute(new_user, new_value)
        db.commit()
        return redirect("/member")
